...
export function SmokeyBackground({ backdropBlurAmount = 'sm', color = '#009870', className = '' }) {
    ...
    useEffect(() => {
        ...
        if (!gl) {
            console.warn('WebGL não suportado — o fundo animado não será exibido, mas o restante da página funciona normalmente.')
            return
        }
        ...
        if (!gl.getProgramParameter(program, gl.LINK_STATUS)) {
            console.error('Erro ao vincular programa WebGL:', gl.getProgramInfoLog(program))
            return
        }
    ...
    const blurClass = BLUR_CLASSES[backdropBlurAmount] || BLUR_CLASSES.sm
